    ...
    @forelse ($cart as $id => $details)
        ...
        <tr>
            <td class="px-6 py-4">
                <div class="flex items-center">
                    <div class="flex-shrink-0 h-12 w-12">
                        <img class="h-12 w-12 rounded-md object-cover" src="{{ $details['image_path'] ? asset('storage/' . $details['image_path']) : 'https://via.placeholder.com/150' }}" alt="">
                    </div>
                    <div class="ml-4">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                <form action="{{ route('sales.cart.update', $id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <input type="number" name="quantity" value="{{ $details['quantity'] }}" class="w-20 text-center border-gray-300 rounded-md" min="1" onchange="this.form.submit()">
                </form>
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <form action="{{ route('sales.cart.remove', $id) }}" method="POST">
                    @csrf
                    @method('DELETE')
    ...
    @empty
        <tr>
            <td colspan="5" class="text-center py-16 text-gray-500">
                ...
                <a href="{{ route('sales.catalog.index') }}" class="text-blue-600 hover:underline mt-2 inline-block">Mulai belanja</a>
            </td>
        </tr>
    @endforelse
</tbody>
